...
read.txt.doc <- function(filename) {
  fulltext = readLines(filename)
  text <- strip.text(fulltext) # Turn into a vector of strings
  return(text)
}


read.directory.of.directories <- function(dirname,verbose=FALSE, first = 26) {
  dirnames = dir(dirname,full.names=TRUE)
  all.stories = list()
  labels = NULL
  for (dir in dirnames) {
    filenames = dir(dir, full.names = TRUE)
    author_name = substring(dir, first = first)
    labels = append(labels, rep(author_name, length(filenames)))
    stories = list()
    for (i in 1:length(filenames)) {
      if(verbose) {
        print(filenames[i])
      }
      stories[[i]] = read.txt.doc(filenames[i])
    }
    all.stories = c(all.stories, stories)
  }
  return(list(all.stories, labels))
}
