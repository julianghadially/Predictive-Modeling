...
After running the bootstrap, we see that the standard deviation is $3084 and the expected return is $100831, or .8% over 20 days. Risk for this model: There is a 95% chance that this portfolio will have a final value above $95,644.29
...
The following histogram depicts the variance. Vertical lines are plotted at 5%, 50%, and 95% of the data.
...
The safe portfolio has an expected return of $100962.10 and Variance of $1995:
...
Risk for the safe portfolio: there is a 95% chance that the final wealth will be above $98237.11.
This is much higher than the even-split porfolio's 95% threshold of 95644.29.
...
The following histogram displays the spread, which is much larger than both the even-split and the safe portfolio. We observe this by looking at the 5% and 95% vertical lines, which lie at $91235.73 and $110938. The implied risk is that 95% of the time, this portfolio will make more than $91235.73
...
comp_portfolio = data.frame('mean' = c(mean_even_split, mean_safe, mean_aggressive), 'sdev' = c(sd_even_split,sd_safe,sd_aggressive), row.names = c('Even Split', 'Safe', 'Aggressive'))

ggplot(data = comp_portfolio, aes(sdev, mean, group = rownames(comp_portfolio), shape = rownames(comp_portfolio))) + geom_point()
```

Based on mean and standard deviation, the safe portfolio is better than the even-split because it has a higher expected return and a lower variance. However, the aggressive portfolio may be a good option for a risk-avid individual who is willing to accept additional risk in return for higher expected returns.
